...
async function greedyRNNT(
  ...
  let state1 = new ort.Tensor("float32", new Float32Array(hiddenDim), [
    1,
    1,
    hiddenDim,
  ]);
  let state2 = new ort.Tensor("float32", new Float32Array(hiddenDim), [
    1,
    1,
    hiddenDim,
  ]);
...
async function ensureModels(
  ...
  _loadP = (async () => {
    ...
    const tokRes = await fetch(`${MODEL_BASE}/tokenizer.json`);
    _tok = (await tokRes.json()) as Tokenizer;
    BLANK_ID = _tok.blank_id;

    onProgress?.(10, `Downloading encoder (~125MB)...`);
    _enc = await ort.InferenceSession.create(
    ...
    onProgress?.(80, "Downloading decoder (~5MB)...");
    _dec = await ort.InferenceSession.create(
      `${MODEL_BASE}/decoder_joint_int8.onnx`,
      {
        executionProviders: eps,
        graphOptimizationLevel: "all",
      },
