...
interface UseTajweedAnalysisReturn {
  ...
  error: string | null;
  analyzeAyah: (audioChunks: ArrayBuffer[], ayahWords: string[]) => Promise<TajweedResult | null>;
...
export function useTajweedAnalysis(): UseTajweedAnalysisReturn {
  ...
  const analyzeAyah = useCallback(async (
    ...
    try {
      ...
      // Base64 encode
      let binary = '';
      for (let i = 0; i < merged.byteLength; i++) binary += String.fromCharCode(merged[i]);
      ...
      for (const v of result.violations) {
        const ex = statuses.get(v.word_index) || { word_index: v.word_index, rules: [], has_violation: false };
        ex.rules.push(v);
      ...
      for (const c of result.confirmations) {
        const ex = statuses.get(c.word_index) || { word_index: c.word_index, rules: [], has_violation: false };
        ex.rules.push(c);
  ...
  return { isAnalyzing, lastResult, wordStatuses, error, analyzeAyah, addAudioChunk, getBufferedAudio, clearBuffer, overallScore };
}
