...
function wordSimilarity(
  ...
  if (spokenNorm === expectedNorm) return spokenHasTashkeel ? 0.85 : 0.90;
  ...
  if (!spokenHasTashkeel) normalizedSim = Math.min(0.90, normalizedSim);
...
export class RecitationSession {
  ...
  matchTranscript(
    ...
    const emptyResult = (): MatchResult => ({
      words: [], wordsMatched: 0,
      newPosition: this.position, position: this.position,
      ayah: this.currentAyah(),
      complete: this.position >= this.totalWords,
      type: "match", totalWords: this.totalWords,
      transcript,
    });
    ...
    for (const spoken of spokenWords) {
      ...
      if (!spokenNorm) { spokenIdx++; continue; }
      ...
      for (let offset = 0; offset < lookahead; offset++) {
        ...
        const sim = wordSimilarity(
          spoken, spokenNorm, spokenHasTashkeel_,
  ...
  static fromQuranWords(quranWords: Array<{
    text:            string;
    ayahNumber:      number;
    globalIndex:     number;
